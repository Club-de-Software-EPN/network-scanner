...
app.on('activate', () => {
  ...
  if (BrowserWindow.getAllWindows().length === 0) {
    network.reactiveChildProcesses();
    createWindow();
  }
});
...
ipcMain.on('discover-network', (event: IpcMainEvent) => {
  network.setEventHandler(event);
  network.discover();
});

ipcMain.on('scan', (event: IpcMainEvent, host: string) => {
  network.setEventHandler(event);
  network.scan(host);
});
